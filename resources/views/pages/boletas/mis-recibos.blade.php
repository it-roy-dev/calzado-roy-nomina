...
<div class="content container-fluid">
    ...
    <x-breadcrumb>
        <x-slot name="title">Mis Recibos</x-slot>
        <ul class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
            <li class="breadcrumb-item active">Mis Recibos</li>
        </ul>
    </x-breadcrumb>
    ...
    <div class="bol-card">
        <div class="bol-card-header">
            <div class="d-flex align-items-center gap-3">
                <div style="width:40px;height:40px;border-radius:10px;background:linear-gradient(135deg,#1e3c72,#2a5298);display:flex;align-items:center;justify-content:center;color:#fff;font-size:15px">
                    <i class="fa-solid fa-file-invoice fa-xs"></i>
                </div>
                <div>
                    <div style="font-weight:700;font-size:14px;color:#0f2456">
                        {{ $boleta->tipo === 'PRIMERA_QUINCENA' ? 'Primera Quincena' : 'Segunda Quincena' }}
                        — {{ $meses[$boleta->nomina->mes] }} {{ $boleta->nomina->anio }}
                    </div>
                    <div style="font-size:12px;color:#64748b">
                        Período: {{ $boleta->nomina->fecha_inicio_periodo->format('d/m/Y') }}
                        al {{ $boleta->nomina->fecha_fin_periodo->format('d/m/Y') }}
                        · Pago: {{ $boleta->nomina->fecha_pago->format('d/m/Y') }}
                    </div>
                </div>
            </div>
        ...
        <div class="modal-content" style="border-radius:14px;overflow:hidden">
            <div class="modal-header" style="background:linear-gradient(135deg,#0f2456,#2a5298);color:#fff;border:none">
                <h5 class="modal-title" style="font-weight:700;font-size:15px">
                    <i class="fa-solid fa-file-invoice me-2"></i> Mi Recibo de Pago
                </h5>
...
function verBoleta(id) {
    ...
    bootstrap.Modal.getOrCreateInstance(document.getElementById('modalVerBoleta')).show();
}
...
function abrirFirma(id) {
    ...
    bootstrap.Modal.getOrCreateInstance(document.getElementById('modalFirma')).show();
...
function guardarYFirmar() {
    ...
    .then(r => r.json())
    .then(data => { if (data.success) confirmarFirma(); });
}
...
window.abrirFirma = abrirFirma;
window.mostrarPad = mostrarPad;
window.limpiarFirma = limpiarFirma;
window.guardarYFirmar = guardarYFirmar;
